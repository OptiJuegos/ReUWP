...
bool PrepareWithDefaults(const char* defaults, DWORD defaults_length) noexcept {
  ...
  // CSIDL_FLAG_CREATE hace que la propia shell cree la carpeta si el perfil es
  // nuevo, que es justo el caso en el que fallaria todo lo de despues.
  ...
  if (::lstrlenW(appdata) >= kMaxAppDataLength) {
    log::Write("APPDATA path is too long");
    return false;
  }

  wchar_t base[MAX_PATH] = {};
  ::lstrcpyW(base, appdata);
  ::lstrcatW(base, L"\\MinecraftPE");
  ...
  if (file != INVALID_HANDLE_VALUE) {
    ...
    log::Write(
        "created roaming MinecraftPE/games/com.mojang/minecraftpe/options.txt");
    return true;
  }
  ...
  if (!IsEmptyFile(options)) {
    log::Write("roaming MinecraftPE options.txt already exists");
    return true;
  }
  ...
  log::Write("regenerating empty roaming MinecraftPE options.txt");
  WriteDefaults(file, defaults, defaults_length);
  return true;
}
