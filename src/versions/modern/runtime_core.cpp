...
void* ReadComPointer(void* object, DWORD offset, const char* what) noexcept {
  ...
  if (!memory::IsReadable(slot, sizeof(void*))) {
    log::Writef("%s: unreadable pointer slot at +0x%X", what, offset);
    return nullptr;
  }

  void* const value = *slot;
  if (value == nullptr || !memory::IsReadable(value, sizeof(void*))) {
    log::Writef("%s: not a readable pointer at +0x%X", what, offset);
    return nullptr;
  }

  void* const* vtable = *static_cast<void* const* const*>(value);
  if (vtable == nullptr || !memory::IsReadable(vtable, 3 * sizeof(void*))) {
...
bool IsRuntimeEntryExecutable(const void* entry, const char* name) noexcept {
  if (entry != nullptr && memory::IsExecutable(entry)) {
    return true;
  }
  log::Writef("modern runtime entry is not executable: %s=%p", name, entry);
  return false;
}
...
bool InitializeRuntimeContext() noexcept {
  ...
  const bool valid =
      context.resource_owner_size != 0 && context.owner_to_renderer != 0 &&
      context.owner_to_device != 0 && context.renderer_to_context != 0 &&
      context.renderer_to_depth_target != 0 &&
      IsRuntimeEntryExecutable(context.alloc_resource_owner, "alloc owner") &&
      IsRuntimeEntryExecutable(context.construct_resource_owner,
                               "construct owner") &&
      IsRuntimeEntryExecutable(context.init_d3d_device, "init D3D device") &&
      IsRuntimeEntryExecutable(context.make_device_resources,
                               "make device resources") &&
      IsRuntimeEntryExecutable(context.make_app_main, "make AppMain") &&
      IsRuntimeEntryExecutable(context.init_render_targets,
                               "init render targets") &&
      IsRuntimeEntryExecutable(context.release_render_targets,
                               "release render targets") &&
      IsRuntimeEntryExecutable(context.app_main_frame, "AppMain frame");
  if (!valid) {
...
bool BindRendererRuntimeContext(void* renderer) noexcept {
  ...
  if (!memory::IsReadable(depth_target_slot, sizeof(void*))) {
    return false;
  }
  ...
  if (context.renderer_to_back_buffer != 0) {
    ...
    if (!memory::IsReadable(back_buffer_slot, sizeof(void*)) ||
        !memory::IsWritable(back_buffer_slot, sizeof(void*))) {
      context.depth_target_slot = nullptr;
      return false;
    }
...
void* GameRenderer(void* owner) noexcept {
  ...
  void** const renderer_slot = reinterpret_cast<void**>(
      static_cast<unsigned char*>(owner) + g_state.context.owner_to_renderer);
  if (!memory::IsReadable(renderer_slot, sizeof(void*))) {
    log::Write("game renderer slot is unreadable");
    return nullptr;
  }

  void* const renderer = *renderer_slot;
...
void* DepthTarget() noexcept {
  void** const slot = g_state.context.depth_target_slot;
  return slot != nullptr && memory::IsReadable(slot, sizeof(void*)) ? *slot
                                                                    : nullptr;
}
...
void* CreateGraphicsOwner(const char* log_name) noexcept {
  ...
  void* storage = alloc(context.resource_owner_size);
  if (storage == nullptr) {
    return nullptr;
  }
  memset(storage, 0, context.resource_owner_size);

  using ConstructFn = void*(__fastcall*)(void* owner);
  const auto construct =
      reinterpret_cast<ConstructFn>(context.construct_resource_owner);
  void* owner = construct(storage);
  if (owner == nullptr) {
    return nullptr;
  }

  if (context.resource_owner_slot != nullptr &&
      memory::IsWritable(context.resource_owner_slot, sizeof(void*))) {
...
void* CreateDeviceResources(const char* holder_log_name,
  ...
  if (context.device_resources_slot != nullptr &&
      memory::IsWritable(context.device_resources_slot, sizeof(void*))) {
...
void RunModernFrame(const FramePlan& plan) noexcept {
  ...
  if (!memory::IsReadable(state.app_main, 0x14) ||
      !memory::IsExecutable(frame)) {
    state.frame_bindings_valid = false;
    log::Write("modern frame stopped: AppMain or frame entry became invalid");
    if (plan.not_ready != nullptr) {
      plan.not_ready();
    }
    return;
