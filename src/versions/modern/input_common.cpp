...
void* EventSink(void* handler) noexcept {
  ...
  if (g_state.cache.event_sink != nullptr &&
      !memory::IsReadable(g_state.cache.event_sink, sizeof(void*))) {
    g_state.cache.event_sink = nullptr;
  }
  if (g_state.cache.event_sink == nullptr) {
    const DWORD offset =
...
bool ResolveCursorFields(void* handler) noexcept {
  ...
  if (g_state.cache.cursor_fields_resolved) {
    return memory::IsReadable(g_state.cache.cursor_mode, 1) &&
           memory::IsWritable(g_state.cache.cursor_mode, 1) &&
           memory::IsReadable(g_state.cache.cursor_command, sizeof(DWORD)) &&
           memory::IsWritable(g_state.cache.cursor_command, sizeof(DWORD));
  }
  ...
  if (!memory::IsReadable(relative_field, 1) ||
      !memory::IsWritable(relative_field, 1) ||
      !memory::IsReadable(command_field, sizeof(DWORD)) ||
      !memory::IsWritable(command_field, sizeof(DWORD))) {
    return false;
  }
...
void* Handler(void* app_main) noexcept {
  if (app_main == nullptr ||
      !memory::IsReadable(Field(app_main, 4), sizeof(void*))) {
    return nullptr;
  }
...
bool PostPointer(void* handler, int x, int y, bool relative,
  ...
  if (!memory::IsExecutable(make_event) || dispatch == nullptr) {
    return false;
  }
  ...
  if (!memory::IsReadable(event, 0x14) ||
