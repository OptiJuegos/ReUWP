...
unsigned char* g_device_context_frame_flag = nullptr;
...
bool ValidateFrameBindings01510() noexcept {
  ...
  g_device_context_frame_flag = nullptr;
  if (void* const context = d3d::DeviceContext()) {
    auto* const flag = static_cast<unsigned char*>(context) + 0x230;
    if (!::IsBadWritePtr(flag, 1)) {
      g_device_context_frame_flag = flag;
    }
  }
...
void ResetDeviceContextFrameFlag01510() noexcept {
  if (g_device_context_frame_flag != nullptr) {
    *g_device_context_frame_flag = 0;
  }
}
...
void RunFrame() noexcept {
  static const modern::FramePlan plan = {
      ...
      nullptr,
      &ResetDeviceContextFrameFlag01510,
      nullptr,
