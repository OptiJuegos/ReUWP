...
bool CallAppMainResize115() noexcept {
  ...
  if (resize_entry == nullptr || !memory::IsExecutable(resize_entry) ||
...
bool ActivateAppMain115() noexcept {
  if (modern::State().app_main == nullptr ||
      !memory::IsReadable(modern::State().app_main, 0x14)) {
    return false;
  }

  auto* const app = static_cast<unsigned char*>(modern::State().app_main);
  void* const app_platform = *reinterpret_cast<void**>(app + 0x04);
  if (app_platform == nullptr ||
      !memory::IsWritable(static_cast<unsigned char*>(app_platform) + 0x08, 1) ||
      !memory::IsWritable(app + 0x10, 1)) {
  ...
  *(app + 0x10) = 1;
  ...
  if (handler != nullptr &&
      memory::IsReadable(static_cast<unsigned char*>(handler) +
                             input_sink_offset,
                         sizeof(void*))) {
    void* const sink = *reinterpret_cast<void**>(
        static_cast<unsigned char*>(handler) + input_sink_offset);
    input_ready = sink != nullptr && memory::IsReadable(sink, 0x1D0);
  }
...
bool CreateAppMain115() noexcept {
  ...
  if (entry == nullptr || !memory::IsExecutable(entry)) {
    log::Write("1.1.5 AppMain factory entry is not executable");
    return false;
  }
...
bool ValidateFrameBindings115() noexcept {
  ...
  if (state.app_main == nullptr || !memory::IsReadable(state.app_main, 0x14)) {
    return false;
  }
  ...
  if (context.app_main_frame == nullptr ||
      ...
      !memory::IsExecutable(context.app_main_frame) ||
      !memory::IsExecutable(context.platform_frame_entry) ||
...
bool HasAppPlatform115(void* app_main) noexcept {
  if (!memory::IsReadable(app_main, 0x08)) {
    return false;
  }
...
void RunPlatformFrame115(void* app_main) noexcept {
  ...
  if (app_platform == nullptr || platform == nullptr ||
      !memory::IsReadable(app_platform, sizeof(void*)) ||
      !memory::IsExecutable(platform)) {
    return;
  }
...
void RunFrame() noexcept {
  detail::EnsureVideoOptionOverrides();
